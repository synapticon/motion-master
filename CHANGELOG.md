...
- **One slow request no longer freezes the whole API.** Every HTTP handler now runs on a worker thread instead of the event loop, so a request waiting on the bus cannot stall the requests behind it. Before this, a firmware transfer holding the control-plane lock for twelve seconds made *every* endpoint unresponsive for its duration — including `/api/version`, which touches no hardware at all, and the procedure-progress poll a user watches an install through, which sat fifteen seconds stale. It affected object-dictionary enumeration, certificate refresh and any slow SDO the same way.
