...
- **Commutation offset measurement**, as `POST /api/devices/{slavePosition}/procedures/commutation-offset-measurement`: measures the commutation angle offset, which the drive stores itself in `0x2001` and marks valid in `0x2009:01`. This is the measurement that commissions an axis and the last step of the sequence — open phase detection, pole pair detection, motor phase order detection, then this. **Motor phase order detection must have been run first**; the drive does not check that, so nothing here can enforce it either, and an offset measured against an unknown phase order is wrong. **What it does physically is configured on the drive, not chosen here**: the method in `0x2009:03` decides both whether the rotor turns and which way the brake goes, so the procedure reads it first and reports the method alongside the offset. The two rotating methods (0 and 1) turn the rotor and need the brake **released** — support any load first, and method 1 additionally needs the gains in `0x2009:04-06` tuned; the stationary method (2) does not turn the rotor and runs with the brake **engaged**, because it cannot hold the load, at the cost of precision. A method that cannot be read or is outside 0-2 fails the run before the drive is touched at all, rather than guessing at the brake. As with motor phase order detection, the restore puts back the brake and the operation mode but deliberately not the measured offset — that is the result, not a side effect.
