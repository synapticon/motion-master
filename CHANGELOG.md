...
- **File transfers wait for a bootloader that is busy writing flash.** FoE now uses its own 10-second mailbox timeout instead of the 700 ms every other mailbox path uses, and it applies to each packet acknowledgement rather than to the whole transfer. A bootloader stops answering while it erases and programs, which is longer than 700 ms: a drive's communication binary failed on every attempt at exactly the old timeout while its application binary — whose packets all acknowledged in time — wrote fine. The cost is that a slave which never answers now stalls one transfer for ten seconds rather than under one.
