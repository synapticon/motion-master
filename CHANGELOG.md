...
- **Motion Master starts auto-tuning itself.** At startup it runs the auto-tuning executable as a child process on loopback, waits for it to answer, and logs one line naming the version and the port: `Auto-tuning 3.1.1 on port 63528 (pid 12345)`. The child's own output goes to `auto-tuning.log`, beside Motion Master's log file and listed by `GET /api/user-cache`. The new `autoTuning` config block sets `enabled`, `binaryPath` and `port`; the defaults run `auto-tuning` from the directory Motion Master itself runs from, on port 63528. A machine without the executable logs a warning naming the path and carries on. Motion Master stops the child when it exits.
