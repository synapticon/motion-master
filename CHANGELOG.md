...
- **Reading a drive's whole object dictionary in one transfer.** `POST /api/devices/{slavePosition}/procedures/read-object-dictionary` runs OS command 21: the drive writes the value of every object it holds to its `fs-buffer`, and the step reports them all with their index and subindex. It is a test of the bulk transfer and a way to cross-check what SDO reads, not a replacement for the parameters page — CoE Complete Access already reads a whole object at a time. It does reach two values SDO cannot: `0x1024`, which is write-only, and `0x1023:01`. The drive sends raw values with nothing marking where one ends and the next begins, so they are split up by the widths read from the device's own object dictionary; when the two disagree the run fails and says where, rather than reporting values that have been shifted.
...
- **A drive's object dictionary is no longer cached when the read lost an entry.** Reading a dictionary is hundreds of mailbox transfers, so the result is written to a file keyed on the drive's vendor, product and revision, and read back on every later scan. That file is never checked against the drive again. A read that lost one entry — a transfer the drive never answered, after the retries — therefore left one host believing a dictionary short of that entry for as long as the file existed, and no other host was missing it. A read like that is no longer written. The log says how many entries were lost, and the next scan reads the dictionary from the drive again. Every entry that was read is correct and is still used for the session. A file already on disk is not corrected by this: delete it on the Console's Storage page, or with `DELETE /api/parameter-cache/{id}`.
- **Reading a whole object dictionary works on an Integro now.** OS command 21 sends the values of the application object dictionary, back to back with nothing marking where one ends. An Integro also answers over CoE for `0x10F8`, the standard Timestamp Object, because the EtherCAT stack in its communication processor serves that one rather than the application. It has no place in the transfer, so counting its eight bytes shifted every value after it and the run failed on the total. That object is now left out of the split, so the step reports one value fewer than the device's dictionary holds entries. The check that catches a real disagreement is untouched: the widths still have to add up to exactly the bytes the drive sent, so an object left out in error fails the run rather than shifting it quietly. A drive that has no such object, a SOMANET Node among them, is unaffected.
- **Reading every parameter of an Integro takes seconds again, and no longer loses any.** A PDO mapping object reserves ten entries and states in its subindex 0 how many are mapped, so a drive with free slots holds storage it is not using. Whether the value of a free slot can be read is the drive's to say: a SOMANET Node answers in under a millisecond, and an Integro answers nothing at all, which costs a 700 ms mailbox timeout to find out. Reading all the parameters of an Integro asked for 53 of them and took 36 seconds. Motion Master now asks once and remembers the answer. The first read of such an entry is still attempted, so a drive that serves them keeps every value; a drive that does not is asked one more time and no further, and the sweep costs about 2.8 seconds, then 2.2 seconds. Nothing about this is tied to a product or a firmware version: it turns on a read that failed. An entry that was not read keeps its declaration and reports no value.
- **A late reply from a drive could corrupt the master's mailbox buffers.** Every mailbox transfer starts by clearing out whatever the drive left in its mailbox, which is how Motion Master recovers from a reply that arrives after its request has timed out. The three transfers that read an object dictionary returned that stale message's buffer to the pool twice. The pool then held one buffer twice over and handed it to two transfers at once, so a request being built and a reply being read shared the same bytes. Buffers were lost from the pool as well, and once it ran empty every mailbox transfer on the bus failed. Reading the dictionary of a drive on a lossy link is where this was reachable, which is the same condition that produces the late reply in the first place.
- **A drive's object dictionary was read short of its unused PDO mapping slots.** SOMANET firmware reports an object's size as the value of its subindex 0, which for a PDO mapping object is how many entries are *currently mapped* rather than how many it holds. So a mapping with free slots — an unmapped `0x1603`, an `0x1A02` using one of its ten — reported itself as that much smaller, and every free slot was missing from `GET /api/devices/{slavePosition}/parameters`. On a SOMANET Integro that was 62 entries. The enumeration now keeps asking for entries past the reported size until the drive refuses, so the free slots appear with their names, types and access flags like any other entry. It costs one extra request per record or array object, and a drive that refuses by not answering at all is detected on the first one and read the old way rather than made slow. Two drives can refuse in different ways. A SOMANET Node aborts the request, which is what the standard asks for. A SOMANET Integro answers it instead, with a description that names nothing: no data type, no bit length and no name. Both now end the search. Only the abort did before, so the Integro was asked for all 255 subindices of every record and array object, and its dictionary came back with about 19500 entries that describe nothing. A dictionary already written to a file is read from the drive again, because a file records what the enumeration of the day produced and this one changed.
...
- **Auto-tuning is installed for you.** The tuning calculations run in a separate executable, which Motion Master starts as a child process. Every install path now downloads it: `./setup.sh` for a tarball, `.\setup.ps1` for the Windows zip, the `postinst` and `%post` scriptlets for the deb and the rpm, and the image build for Docker. The file installs as `auto-tuning` next to the server binary. It is about 65 MB and it changes a few times a year, so it is downloaded once from a rolling release instead of shipped in every Motion Master release. It is also only needed while a drive is being commissioned, which is the other half of why it is optional: a system in operation never calls it. A machine that already has the file keeps it, so an upgrade does not download it again. If the download fails, each install path says so and continues, and Motion Master runs without auto-tuning. Only the Docker image requires the file: an image build that cannot reach the release fails.
- **Motion Master starts auto-tuning itself.** At startup it runs the auto-tuning executable as a child process on loopback, waits for it to answer, and logs one line naming the version and the port: `Auto-tuning 3.1.1 started on port 63528 (pid 12345)`. The child's own output goes to `auto-tuning.log`, beside Motion Master's log file and listed by `GET /api/user-cache`. The new `autoTuning` config block sets `enabled`, `binaryPath` and `port`; the defaults run `auto-tuning` from the directory Motion Master itself runs from, on port 63528. A machine without the executable logs a warning naming the path and carries on. Motion Master stops the child when it exits, and says how that went: an ordinary stop is one info line, a process that had to be killed is a warning, because that is the case where something could be left holding the port. A call that cannot reach the process also logs a warning once, so a child that dies mid-session shows up in the server log rather than only in one client's console.
- **Auto-tuning is on the API.** `POST /api/auto-tuning/run` forwards a `{ "run": ..., "data": ... }` request to the auto-tuning process and returns its reply unchanged, so every function it offers is reachable through one endpoint. Measurement data travels in the body: `identify_plant_model` takes the chirp CSV as `data.csv` and the bode data comes back inline, so no file is written anywhere. `GET /api/auto-tuning` reports whether the process is enabled, installed, started, and which version answered — enough for a client to say why a call would fail. `GET /api/auto-tuning/swagger.yml` serves that program's own API description. `swagger.yml` carries the request and result schemas, so `@synapticon/motion-master-client` is typed for every function; a call that rejects its inputs still answers 200 with an `error` property, which is that program's convention and is passed through as it is. `run: exit` is refused with 400, because it would stop the process for every other client.
- **Console: an Auto-Tuning page under Tools.** Calls every auto-tuning function from the browser, with a prepared example for each — both position-controller tuning rules and both notch cases included — over an editable request body, so a function the page does not list yet can still be called by editing `run`. The response is shown as it arrives, and a refusal is marked as a failure even though its status is 200. `identify_plant_model` takes its chirp measurements from a file picker, whose text is sent in the request body. When auto-tuning is not running the page says which of the reasons applies and what to do about it, rather than showing a form that fails.

## [6.0.0-alpha.81] - 2026-08-20

### Fixed

- **`@synapticon/motion-master-client` is on npm again.** The `6.0.0-alpha.80` server binaries shipped, but the client library of that version never reached the registry: the release workflow moved to npm trusted publishing and the publish step authenticated with a placeholder credential instead of the token it exchanges for. Install `6.0.0-alpha.81` or later; there is no `6.0.0-alpha.80` on npm.
...
- **`Device::value(index, subindex)` is now `Device::parameterValue(index, subindex)`.** It differed from the real-time `value<T>()` by one template argument, so whether a call took a lock was invisible at the call site. The real-time pair keeps the short name.
- **`DeviceManager::readDeviceParameter`, `readDevicePdoMapping` and `value` are gone.** Each only resolved a bus position and forwarded to the same method on `Device`, which `deviceAt(position)` now does in two lines. `readAllDeviceParameters`, `writeDeviceParameter` and `deviceParameterView` stay, because they inject the manager's own configuration or compose two steps so a re-enumeration cannot land between them.
- **`CyclicTask::execute` is now `noexcept`.** A task that throws terminates at the throw site, so the report names the line that threw rather than the loop that ran it. Add `noexcept` to your overrides; a task that can fail reports it through a value it publishes or a log line, never by throwing.
- **A retired device set is freed when its last holder releases it.** A `DeviceHandle` keeps its device alive for as long as you hold it, so a rescan neither waits for you nor invalidates you, and memory does not grow with rescans. A cyclic task still resolves its devices each cycle and caches nothing across cycles; `topologyGeneration()` is how it notices a rescan happened.
