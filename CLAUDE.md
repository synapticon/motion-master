...
main.cc  (composition root — the only place concrete types are instantiated; no `App` class yet)
 ...
 │           └── [planned: SineWaveTask + TrajectoryCyclicTask + other RT target generators, idle until activated; fed depth-1 latest-wins mailboxes (per-Device, or an RtMailboxPool on DeviceManager for coordinated multi-axis moves)]
...
- **RT cyclic procedures** *(planned — not yet in code)*: SineWave / trajectory / profile / ramp generators — anything that must write a target into the output region every cycle — will be `CyclicTask`s registered up front and *idle until activated* via a control block, exactly like `ProcessDataTask` is a no-op until an image is published. The control block is a **depth-1 latest-wins mailbox** — newest intent supersedes, never a FIFO (a chained/queued move, if ever needed, is a command queue added *beside* the slot, not a redesign of it). For a **per-device generator** (SineWave) it is a lock-free published `SineWaveParams` held on `Device` (`unique_ptr`, like `parametersMutex_`), one slot per device being the fixed pool; for a **cross-device coordinated move** (multi-axis trajectory) it is a single immutable program with a column per axis and one shared cursor, held in a fixed pool on `DeviceManager` (it cannot live on any one `Device`), with an axis in at most one active program. **The launch lives off the scheduler, not the HTTP handler:** for one device on the view (`Cia402Drive::startSineWave(...)/startTrajectory(...) → expected<>`), for a coordinated move on a node-layer coordinator that *composes* per-axis views; either validates and does the op-mode/enable handshake synchronously off-RT, then stores the block with `active = true` — with fixed membership a "launch" is just a control-block write. The single task (holds `DeviceManager&`) iterates devices/programs each cycle, runs after `ProcessDataTask`, and writes the output slots directly (on-RT); RT-only scratch (phase, cursor) stays off the published block. Both directions generalize to two task-specific functions over shared plumbing — inbound `RtMailboxPool<T>` + `launchRtTask(validate, build)`, outbound the `NotificationBus` + `Source{changeToken, render}`; the RT thread never touches the WebSocket (the seam is a `std::function` publish callback wired in `main.cc`, like `MonitoringManager` — `node` never names `WebSocketServer`). See NEXTGEN.md Session 2026-07-09 ("Multi-axis coordinated trajectory … depth-1 latest-wins").
