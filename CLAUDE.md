...
App  (composition root, owns everything)
 ...
 ├── WebSocketServer  (own port 8444 + loop; realtime channel: monitoring/notifications/progress out, subscribe + output staging in)
...
Motion Master binds to `127.0.0.1:8443` (HTTP API) and `127.0.0.1:8444` (realtime WebSocket), on separate event loops/threads. The PWA at `https://motion-master.synapticon.com` connects to `https://local.motion-master.synapticon.com:8443` (HTTP API) and `wss://local.motion-master.synapticon.com:8444` (WebSocket). The WS port is configurable via `--ws-port`. The DNS record `local.motion-master.synapticon.com A 127.0.0.1` resolves to localhost. CORS is set to `Access-Control-Allow-Origin: https://motion-master.synapticon.com`.
