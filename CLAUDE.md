...
**Off-loopback deployment (Raspberry Pi appliance / any LAN host) — code as-built, DNS still outstanding.** A server the browser reaches over the network can't use `local.…` (it pins to `127.0.0.1`), can't get a public cert for a private IP or `.local`, and can't fall back to self-signed (the PWA's cross-origin `fetch()` gives no click-through). The fix is the same trick one level up: names under `ip.motion-master.synapticon.com` encode the address in their leftmost label (`192-168-1-50.ip.… → 192.168.1.50`), and the bundled cert's `*.ip.…` wildcard covers all of them, so nothing is registered per device. **In code:** `server.bindAddress` (above), the two-SAN cert + SAN-based validation (above), `lanHostname()` in `@synapticon/motion-master-client` (`src/lan.ts`, maps an IPv4 literal to the dashed name and passes hostnames through), applied by the console's `ConnectionPage` on commit, and `dnsNames` on `GET /api/cert` surfaced as **Valid for**. **Not in code — required before it works end to end:** the `ip.…` subdomain must actually resolve. Preferred is a **dnsBOX-native** scripted zone (PowerDNS Lua/pipe) — no delegation, and `_acme-challenge.ip.…` stays an ordinary CNAME in the same zone; otherwise NS-delegate to **CoreDNS + `template` plugin**, which synthesizes the A answers *and* can serve the ACME CNAME. Stock **sslip.io cannot** — it answers only A/AAAA/NS/SOA/MX, so a zone delegated to it has nowhere to put the DNS-01 challenge. Discovery (mDNS/Avahi on the device) and a flashable Pi image remain open. See NEXTGEN.md Session 2026-07-31 (as-built) and 2026-07-24 / 2026-06-12 (design, rejected alternatives).
