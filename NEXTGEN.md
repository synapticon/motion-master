...
The cause was ours, in `SoemFieldbusDriver::transitionToState`. Leaving BOOT for PRE-OP makes the bootloader hand over by *restarting the device*, and the restart wipes the mailbox sync managers the master programmed moments earlier — but they were programmed exactly once, on the first attempt, and the slave was struck off `bootMailboxSlaves_` at that moment. So the restarted device came back in INIT with unusable mailbox configuration and nothing could ever reprogram it: the resend loop re-issues the AL state request and nothing else. It sat there until the timeout with **no AL status code**, which is why it read as a mystery — a slave that *refuses* a state says why; a slave that cannot act on the request at all says nothing. The slave is now kept on the list until PRE-OP is genuinely reached, and the resend path reprograms the sync managers whenever it sees a still-marked slave back in INIT.
