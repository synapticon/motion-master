...
**Still owned outside the repo (Path B, unchanged).** The `ip.…` subdomain needs to resolve before any of this works end to end: a synthesizing responder under `ip.motion-master.synapticon.com`, plus a `_acme-challenge.ip.…` CNAME so the wildcard can be issued. Preferred first step is a **dnsBOX-native** scripted zone (PowerDNS Lua records or the pipe backend) — no delegation, no host to run, and `_acme-challenge.ip.…` stays an ordinary CNAME in the same zone. Failing that, NS-delegate to **CoreDNS with the `template` plugin**, which synthesizes the A answers in a few lines *and* can serve the ACME CNAME; stock **sslip.io** cannot, because it answers only A/AAAA/NS/SOA/MX — no TXT, no CNAME — so a delegated zone would have nowhere to put the challenge. Both challenge names CNAME to the **existing** acme-dns account: it keeps two rolling TXT records, this issuance needs exactly two, and that is also the ceiling — a third SAN would evict one and require a second account.
...
**Status:** as-built for everything in this repo (280/280 tests, console typechecks). Not yet exercised end to end — that waits on the DNS work above and a Pi on the bench.
