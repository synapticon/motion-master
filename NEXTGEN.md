...
## Session 2026-08-24 — `NotificationBus` lands, and `BusHealthReporter` becomes its first source (as-built)

Built the outbound half of the mailbox discipline specced in Session 2026-07-14, and folded the one standalone reporter into it. `libs/node/notification_bus.{h,cc}` is the pump; `apps/motion_master/bus_health_reporter.{h,cc}` is gone, replaced by a `busHealthSource(DeviceManager&)` free function in `bus_health_source.{h,cc}` that returns a `Source`. The class became a function, which is the whole point of generalizing: a feature contributes two lambdas and owns no thread.

**One topic for every notification, not one per source — and the reason is a library limit, not a preference.** The 2026-07-14 sketch gave `Source` a `topic`, which reads as the tidier design and would have been wrong here. **uWebSockets 20.77's `TopicTree` is a flat map of exact names, with no MQTT-style wildcard** (older uWS had one; this version does not). So with a topic per source, "give me every notification" is not expressible: a client must enumerate names, and a source added in a later version silently reaches no existing client. That is a quiet failure mode traded for filtering nobody asked for. Notifications are low-rate lifecycle events; a source high-rate enough that a client should not pay for it is a monitoring stream, not a notification.

**Event names are kebab-case, and each is a `constexpr std::string_view` beside the source that sends it.** The first cut wrote `short_working_counter` inline, matching the `slaves_changed` example in `CLAUDE.md` and the client's doc comment — which turned out to be a placeholder from the 2026-05-18 protocol sketch that nothing implements, and inconsistent with everything that does: procedure names are `offset-detection` and `firmware-installation`, step ids are `extra-files` and `app-firmware`. Matching the example rather than the codebase is the whole mistake. It is now `kShortWorkingCounterEvent = "short-working-counter"` in `bus_health_source.h`, declared the way procedure names are, because an event name is protocol — a client switches on it, so it has to survive a rewrite of the message around it. Nothing enforces the case, so the convention is written into `CLAUDE.md`'s WebSocket section: a new source is where it would drift.

So `kNotificationTopic` is `"notifications"`, every source publishes to it, and the client's `onNotification` ref-counts a subscription to it exactly the way `subscribe` does for monitoring — first listener subscribes, last unsubscribes, `onopen` re-subscribes across a reconnect. Clients still tell events apart by `data.event`, which was always in the payload. The seam is `std::function<void(std::string topic, std::string json)>`, identical to `MonitoringManager::setPublish`, and the bus builds the envelope so the message shape lives in one place.

*Broadcast was the first cut, and it was wrong for a subtler reason than it looked.* It works, and it needs no client change. But it makes notifications the one message class outside the protocol's topic model, and it leaves `WebSocketServer::broadcast` — reach every client regardless of subscription — spent on routine traffic. `broadcast` stays in the class with no caller, documented: the day something must be said to everyone regardless of what they asked for (the server is going down, this build is incompatible), a topic is the wrong mechanism.

**A source's cadence is its own, and one number does two jobs.** The sketch had a single 20 ms poll for the whole bus. Two things were wrong with that. It is a latency floor, not a send rate — a poll that finds nothing changed sends nothing — so it never caused extra messages; but it did cost fifty wakeups a second forever, on a machine also running a 1 kHz RT loop, and for the only source that exists it bought nothing at all. And a source whose counter is bumped from the RT loop bumps it *every cycle* while the fault lasts, so some rate limit had to exist on day one.

Both wanted the same field. `Source::interval` is how often that source is *looked at*, which is simultaneously its latency floor and its ceiling on messages — a counter read once a second cannot be reported more often than that, so a separate `minInterval` collapses into it. The bus sleeps until the earliest due source, exactly what `MonitoringManager`'s sampler already does with `wait_until(nearest deadline)`. A future trajectory source can ask for 20 ms and nobody else pays for it.

Bus health reads **every second**. The reporter it replaced used 10 s, which was chosen when the alternative was a dedicated thread and nothing was watching the output but a log file. It is too slow now: the fault reaches a person's screen, and someone who has just caused a short working counter should see it while they are still looking at what caused it. A second is still three orders of magnitude below the 1 kHz rate the counter is bumped at, so it does not report at the rate of the fault — and nothing is lost to the gap, because the count is cumulative and carries the time of the first and last cycle in it.

The ordering rule survives the collapse: **the last-seen mark advances only when a message actually goes out**, so a change arriving between readings is reported whole at the next one rather than swallowed. Same reason `render` returning `nullopt` leaves the change outstanding.

**A source may log as well as publish, and that is per-source, not a rule.** The reporter warned into the log, and a support log showing short-working-counter cycles from an unattended machine is worth more than a notification to a client that was not connected. So `busHealthSource`'s render does both: `spdlog::warn` with two *ages*, and a payload with two *absolute* epoch-microsecond times. The split is on purpose — a log line is read where it sits and has to carry its own reference point, while printing an absolute time beside spdlog's local-time prefix would have to pick a zone; a client has a clock and the rest of this API in epoch microseconds.

**The RT write got tightened while it was open.** It was three relaxed stores with the counter bumped by `fetch_add` *before* `firstShortWkcNs` was written, so a reader catching the very first short cycle could see count 1 beside a zero timestamp. Benign — the render already treats 0 as unknown, and the next poll corrects it — but the bus makes the counter a change signal, and a change signal that arrives before the data it announces is the wrong shape to leave in place. Now: read the counter (this thread is its only writer while the bus exchanges), write both timestamps relaxed, then `store(count + 1, release)`. Readers load it with `acquire` first. **The counter is the publish fence**, which is the same discipline the spec asked of every future source, so the first one should not be the exception.

`DeviceManager::shortWkcCycles()` is new for the same reason: `processImageInfo()` takes `processDataMutex_` shared and walks both mapped-object lists, which is fine for a render that fires on change and far too heavy to poll as a token twenty times a second.

**What did not change.** The bus names no server type. `node` still never names `WebSocketServer`. Membership is fixed before `start()`, the same rule `CyclicTask` follows, which is what lets the poll thread read the registry with no lock — and `start()` seeds every last-seen mark, so a producer already counting at boot raises nothing for what it had already recorded. Procedures remain off the bus entirely (Session 2026-08-16): they run off-RT and are HTTP-poll-only.
