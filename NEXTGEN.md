...
## Session 2026-07-14 — Hoist `CyclicTask` into `libs/core`: an RT task is a node object that owns its own inbox — no adapter, no controller, no `DeviceManager` pool (design)
...
**What it collapses.** A runtime RT procedure becomes **one self-contained node object** that owns its inbox, is its own tick, and self-registers:
...
libs/node/trajectory_task.h ← class TrajectoryTask : public CyclicTask
                                owns RtMailboxPool   (the inbox — lives on the task that reads it)
                                arm(program)         (launcher writes here: node-to-node, no barrier)
                                execute(ctx)         (plays one point per cycle — IS the tick)
                                publishTo(bus)       (owns the `revision` atomics → owns render)
apps/motion_master/main.cc  ← TrajectoryTask trajectory{deviceManager};
...
- **`DeviceManager` does not own the pool.** "It cannot live on any one `Device` → therefore `DeviceManager`" skipped the real answer: *→ therefore the task.* The task is neither a `Device` nor `DeviceManager`. Multi-axis is unaffected — a coordinated program spans devices but lives in the *task's* pool; the task holds `DeviceManager&` and re-resolves each axis per cycle via `findDevice`, exactly as before. This supersedes the pool-on-`DeviceManager` model in Sessions 2026-07-09/07-13.
...
- **No app-layer adapter.** `TrajectoryTask` registers itself with `GameLoop` directly; the launcher (`Cia402Drive::startTrajectory(TrajectoryTask&, spec)`) arms it node-to-node.

**Costs, honestly.** It touches shipped code: `ProcessDataTask` **relocates** `apps/ → libs/node` (still ~6 lines, but no longer a cross-boundary adapter — a plain node task beside `DeviceManager`), and `game_loop.h`'s include repoints to `core/cyclic_task.h`. It also *commits `node` to housing RT tasks* — a directional statement, but the one already taken by the RT-callable-view plan (Session 2026-07-09): task **logic** (play a buffer into output slots) is a motion concern that belongs in node; task **scheduling** (`GameLoop`) stays in `apps/`. Clean three-way split — interface in core, logic in node, scheduler in app. The HTTP launch still needs **one** wire (the trajectory route handler needs `DeviceManager&` to build the view and `TrajectoryTask&` to arm it — one added reference on the built-in route surface, or one `startTrajectory` callback in `main.cc` mirroring `setGameLoopPeriod`), not the JSON-building `addSource` loop rejected in the NotificationBus session above.
