...
**Surfacing is deferred, but the sensor is built now.** `waitForNextCycle()` returns `uint64_t skipped` (not `[[nodiscard]]`, so `jitter_bench` ignores it freely); `GameLoop` accumulates it into a silent `overruns()` counter (relaxed atomic, sibling to `tick()`) — **no logging, no notification on the RT path** (a non-RT dev box skips constantly; unthrottled logging would be pure noise). The counter is the series the **planned master-side frame/WKC health timeline** (Session 2026-06-01) will sample — an intermittent RT stall is precisely the kind of fault a point-in-time reading misses. Building the counter now avoids retrofitting the hot path when that timeline lands.
...
- **Real-time** ("stay on schedule"): advance the cursor by **1+skipped** (i.e. `cursor = gridTick - startTick`). Preserves the **timing** — wall-clock position honoured, waveform frequency stays accurate, multi-axis stays coordinated — at the cost of **continuity**: the setpoint jumps at resume (a velocity/accel spike, possible following-error). A **clamp** on the per-cycle setpoint delta (cap to a safe velocity) is a safety net inside this mode, not a third mode.

**Plumbing follows from that: defer it until Real-time lands.** Because Sequential needs no timing context, the only reason to change `CyclicTask::execute()` is Real-time. When it lands, pass a small immutable `CycleContext { uint64_t gridTick; uint64_t skipped; }` into `execute()` (push, not a pull of `GameLoop::overruns()` — the scheduler telling tasks about time is the scheduler's job). Prefer `gridTick` (periods elapsed since `run()`, jumps by 1+skipped on a stall — derivable today as `tick_ + overruns_`) as the primary field: a time-parameterised task computes its state as a **pure function of absolute grid time** (`cursor = gridTick - startTick`), which is the *same* robustness principle as the absolute-deadline timer — no per-cycle accumulation, skips handled by the index simply jumping. `skipped` rides alongside only for anomaly policy (e.g. the opt-in strict mode). `ProcessDataTask` ignores the context.
