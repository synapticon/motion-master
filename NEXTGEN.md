...
**Verified end to end on an ACTILINK-S.** The full chain in one run: BOOT, application binary in 11.7 s, communication binary through two `FOE_BUSY` resends in 2.4 s, the handover restart taking the device off the bus for ~8 s, the master noticing it had lost its station address and re-assigning it at its wire position, the mailbox reprogrammed, and PRE-OP reached 100 ms later. **One device recovered by itself; nothing else on the bus was touched.**
