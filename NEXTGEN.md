...
- **The seqlock is no longer the monitoring source.** It stays only for *point* reads — current-value lookups and the SDO health-gating in `readPdo`. The ring is the single source of truth for both the full history *and* the live plot. (Earlier in this session I mis-stated that live monitoring would read a "recent window" off the seqlock — corrected: live monitoring reads the ring, losslessly.)
...
### ⏸ DEFERRED (2026-06-08) — file dump of the recorder, revisit in a few days (~mid-June 2026)

**Parked, not cancelled.** The dump is the genuinely complicated part (binary file format, surviving image teardown, the re-map/layout interaction) and is cleanly separable from the live recorder, which proceeds without it. We return to this in several days. The design worked out so far, to resume from:
...
**Open / next (live recorder, active):** where the ring lives (a new member on `ProcessData`, written from `exchangeProcessData` after the snapshot publish) and its exact allocation point (`configureProcessData`), plus wiring `MonitoringManager` to read the ring instead of interval-sampling the seqlock. **Open / next (deferred dump):** everything in the parked section above.
...
- **No `/tmp` defaults in config.** The deferred dump's `dumpDir` is *not* added yet, and when it is its default must be cross-platform (`std::filesystem::temp_directory_path()`), never a hardcoded `/tmp` (Windows has none).
...
**Settings are config-file-ONLY — they have no CLI flags (settled & implemented 2026-06-09; reverses the earlier "CLI overrides config" plan).** A tunable setting is set *only* through the JSONC file; the corresponding `--port`/`--ws-port`/`--cert`/`--key`/`--driver`/`--adapter`/`--log-level`/`--cors-origin`/`--no-cert-update` flags were **removed**. The CLI surface is now just actions + cert-fetch sources: `--help`, `--version`, `--list-adapters`, `--config`, `--open`, `--update-cert`, `--cert-url`, `--key-url`. This dropped the whole CLI-override layer (no precedence merge, no two-phase pre-pass) — `parseOptions` parses the CLI actions, then loads the file into `opts.config` and resolves the adapter. The settings schema (mapped 1-1 to the `Config` struct tree, `config.h`):
  ```jsonc
  {
    ...
    // "recorder": { "historySeconds": 300 }  — deferred with the recorder, not added yet
  }
  ```
