...
**CoE capabilities surfaced (`ECT_COEDET_*`).** SOEM reads a device's CoE mailbox capability byte (`ec_slavet.CoEdetails`) from EEPROM during `ecx_config_init` — including `ECT_COEDET_SDOCA`, the device's **advertised** Complete Access support. Decoded (via one `decodeCoeCapabilities` helper) into a new `mm::comm::CoeCapabilities` (`sdo`, `sdoInfo`, `pdoAssign`, `pdoConfig`, `uploadAtStartup`, `completeAccess`) and surfaced two ways: as `SlaveConfig.coe` on `GET /api/bus-config` — rendered on the **Configuration** page's slave card, right beside the mailbox `protocols` it refines (swagger `CoeCapabilities` schema + regenerated TS client), the natural home next to the other static EEPROM/ESC config; and, kept on `SlaveInfo`/`Device` (`coeCapabilities()`), logged at scan as one debug line per device. It is **not** on the device JSON (identity only) and deliberately **not** wired into CA detection: the flag is an EEPROM advertisement firmware can get wrong in either direction, so gating on it risks a false-negative that disables CA on a capable-but-non-advertising drive — **confirmed on SOMANET**, which reports `completeAccess=false` yet CA works (the runtime probe succeeds). The probe/fallback stays authoritative; the flag is exposed for diagnostics only, with a UI tooltip spelling out that it is advertised-only and may read false while CA works. An earlier revision put `coe` in the device header/JSON; it was moved to the Configuration page (bus-config) as the correct home — six advisory flags don't belong in the per-device header.
