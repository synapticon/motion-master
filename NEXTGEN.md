...
**Plumbing: only Real-time needs it — but built now, ahead of its consumer.** Because Sequential needs no timing context, the only *behavioural* reason to change `CyclicTask::execute()` is Real-time. The plumbing was nonetheless landed now (2026-07-13) — it is far cheaper to change the one-line `execute()` signature and its single implementor (`ProcessDataTask`) while they're trivial than to retrofit it once several tasks exist. `execute()` now takes a small immutable `CycleContext { uint64_t gridTick; uint64_t skipped; }` (defined in `cyclic_task.h`), pushed by `GameLoop` (push, not a pull of `GameLoop::skippedCycles()` — the scheduler telling tasks about time is the scheduler's job). `ProcessDataTask` ignores it; no other task exists yet. `gridTick` is the primary field — periods elapsed since `run()`, jumps by 1+skipped on a stall, computed in `run()` as executed-iterations + cumulative-skipped (i.e. `tick() + skippedCycles()`); note this differs from `GameLoop::tick()`, which counts only executed iterations. A time-parameterised task computes its state as a **pure function of absolute grid time** (`cursor = gridTick - startTick`), the *same* robustness principle as the absolute-deadline timer — no per-cycle accumulation, skips handled by the index simply jumping. `skipped` rides alongside only for anomaly policy (e.g. the opt-in strict mode).
