...
**First hardware run, and it found the one thing the fake driver could not.** On an ACTILINK: BOOT entered, warm-up served, `app_firmware.bin` (405504 B) written in 11.5 s — and then `com_firmware.bin` refused in under a second, five times over, before the always-attempted exit put the device cleanly back in PRE-OP. The failure was fast and generic (the slave answering with an FoE Error packet, not a timeout), which is the signature of a bootloader that is no longer there to talk to: **committing a firmware image takes the device out of BOOT**, and nothing re-checked between transfers. The previous generation guarded exactly this — `flash_bin_file` re-read the state before *each* binary and re-entered BOOT with a fresh warm-up — and the guard was lost in the rewrite. Two things worth keeping from it: the retry loop made it worse rather than better, because retrying a request the device cannot serve in its current state can only fail identically five times, so **the state check belongs inside the retry, not before it**; and the always-attempted exit from BOOT earned its keep on its first real failure, which is precisely what it was written for. Fixed by `ensureBootReady` before every transfer and every retry — one AL status read when the device stayed put — with a regression test that drops the fake out of BOOT after the first write and was confirmed to fail without the fix.
