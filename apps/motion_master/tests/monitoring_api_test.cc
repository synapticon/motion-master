...
TEST(ParseMonitoringRequest, ParsesFullRequest) {
  const auto body = nlohmann::json::parse(R"({
    "topic": "left-leg", "name": "Left Leg", "interval": 1000,
...
TEST(ParseMonitoringRequest, NameIsOptional) {
  const auto body =
      nlohmann::json::parse(R"({"topic":"t","interval":10,"parameters":[[1,8240,1]]})");
  auto m = parseMonitoringRequest(body);
  ASSERT_TRUE(m.has_value());
  EXPECT_FALSE(m->name.has_value());
}

TEST(ParseMonitoringRequest, LegacyBufferSizeIsIgnored) {
  // bufferSize was removed; an older client that still sends it must not be rejected for it.
  const auto body = nlohmann::json::parse(
      R"({"topic":"t","interval":10,"bufferSize":16,"parameters":[[1,8240,1]]})");
  EXPECT_TRUE(parseMonitoringRequest(body).has_value());
}
...
TEST(ParseMonitoringRequest, RejectsBadShapes) {
  ...
  EXPECT_FALSE(bad(R"({"interval":10,"parameters":[[1,1,1]]})"));                  // no topic
  EXPECT_FALSE(bad(R"({"topic":5,"interval":10,"parameters":[]})"));               // topic not str
  EXPECT_FALSE(bad(R"({"topic":"t","parameters":[[1,1,1]]})"));                    // no interval
  EXPECT_FALSE(bad(R"({"topic":"t","interval":10})"));                             // no parameters
  EXPECT_FALSE(bad(R"({"topic":"t","interval":10,"parameters":[[1,1]]})"));        // arity
  EXPECT_FALSE(bad(R"({"topic":"t","interval":10,"parameters":[[1,70000,1]]})"));  // out of range
}
