...
inline constexpr auto kAlStatusCodes = std::to_array<AlStatusCode>({
    ...
    {0x0002, "No Memory",
     "Slave has insufficient memory to complete the requested operation", true},
    {0x0003, "Invalid Device Setup", "The device setup is invalid", true},
    {0x0005, "Reserved", "Reserved due to compatibility reasons"},
    {0x0011, "Invalid requested state change",
     "The requested state transition is not allowed", true},
    {0x0012, "Unknown requested state",
     "The requested state is not a valid EtherCAT AL state", true},
    {0x0013, "Bootstrap not supported", "The slave does not support the Bootstrap state", true},
    {0x0014, "No valid firmware",
     "No valid firmware is present — the slave needs firmware flashed", true},
    {0x0015, "Invalid mailbox config (BOOT)",
     "Mailbox configuration is invalid for BOOT state", true},
    {0x0016, "Invalid mailbox config (PRE-OP)",
     "Mailbox configuration is invalid for PRE-OP state", true},
    ...
    {0x0026, "Inconsistent settings",
     "Application and mailbox settings are inconsistent", true},
    ...
    {0x0029, "FreeRun needs 3buffer mode",
     "FreeRun mode requires 3-buffer SyncManager mode", true},
    ...
    {0x0030, "Invalid DC SYNC config",
     "Invalid Distributed Clocks SYNC configuration", true},
    {0x0031, "Invalid DC latch config",
     "Invalid Distributed Clocks latch configuration", true},
    ...
    {0x0035, "Invalid DC sync cycle time",
     "Invalid Distributed Clocks sync cycle time", true},
...
constexpr bool isAlStatusCodeTerminal(uint16_t code) {
  for (const auto& entry : kAlStatusCodes) {
    if (entry.code == code) {
