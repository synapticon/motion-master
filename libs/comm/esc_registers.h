...
inline constexpr auto kEscRegisters = std::to_array<EscRegister>({
    ...
    {0x0007, 1, "port_descriptor",
     "Port type per port (2 bits each: 0=not impl, 1=not used, 2=EBUS, 3=MII)"},
    ...
    {0x030C, 1, "ecat_pu_error_counter",
     "ECAT processing unit error counter (frame processing failures)"},
    ...
    {0x0400, 2, "watchdog_divider",
     "Watchdog clock prescaler (basic tick = 25 ns × (divider + 2))"},
    ...
    {0x0918, 2, "dc_receive_time_ecat_unit",
     "DC: local time at ECAT processing unit (ns, 32-bit lower word)"},
