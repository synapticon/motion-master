...
// Resolves a device by position and binds a validated CiA402 view, under a lock the caller already
// holds. Returns the same error strings the individual steps produce so callers stay uniform.
std::expected<Cia402Drive, std::string> resolveCia402Drive(DeviceManager& dm,
                                                           uint16_t slavePosition) {
...
std::expected<Cia402Status, std::string> DeviceManager::getCia402Status(uint16_t slavePosition) {
  // Shared lock, like the other position-based methods: keep the device pointer valid against the
  // exclusive rebuilders while the drive's reads are serialised per transaction by the driver.
  std::shared_lock lock(busMutex_);
  auto drive = resolveCia402Drive(*this, slavePosition);
  if (!drive) {
...
std::expected<Cia402Status, std::string> DeviceManager::setCia402OperationMode(
    uint16_t slavePosition, cia402::OperationMode mode) {
  std::shared_lock lock(busMutex_);
  auto drive = resolveCia402Drive(*this, slavePosition);
  if (!drive) {
    return std::unexpected(drive.error());
...
std::expected<Cia402Status, std::string> DeviceManager::runCia402Command(
    uint16_t slavePosition, Cia402Command command, std::chrono::milliseconds timeout) {
  // Held for the command's whole duration — including enable()'s multi-second polling walk. The
  // walk sleeps between polls but each read/write it issues takes the driver's control-plane lock
  // only per transaction, so holding this shared lock blocks only the exclusive rebuilders, never
  // the RT loop or the WebSocket (matching initializeDeviceParameters' multi-second reads).
  std::shared_lock lock(busMutex_);
  auto drive = resolveCia402Drive(*this, slavePosition);
  if (!drive) {
    return std::unexpected(drive.error());
...
std::expected<void, std::string> DeviceManager::setCia402Target(uint16_t slavePosition,
