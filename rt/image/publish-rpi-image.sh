...
# the size.
#
# One key, overwritten each time. The documentation points at a URL that has to
# keep working, and a version in the filename would move it on every release.
# The version is inside the image, and the .sha256 beside it says which build a
# given download is.
set -euo pipefail
...
if [ "$ASSUME_YES" = false ]; then
    printf 'Upload and overwrite %s? [y/N] ' "$URL"
    read -r reply
    case "$reply" in
        y | Y) ;;
        *) die "cancelled" ;;
