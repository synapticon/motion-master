...
export default function EscRegistersPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Meta" title="ESC Registers" />
        ...
        {registersQuery.isSuccess && (
          <>
            <p className="text-xs text-grey-600 mb-4 max-w-3xl">
              Well-known EtherCAT Slave Controller (ESC) registers, assembled from the{' '}
              Beckhoff EtherCAT ESC datasheet (Section II) and{' '}
              <span className="font-mono">ETG.1000.4</span>. FMMU and SyncManager entries list the
              full block for the first instance; subsequent instances follow at fixed strides
              (FMMU: 16 bytes, SM: 8 bytes).
            </p>
