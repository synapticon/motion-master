...
import { WebSocketClient } from '@synapticon/motion-master-client'
...
/// Thin React wrapper over the SDK's framework-agnostic `WebSocketClient`. Reconnection, per-topic
/// subscribe/unsubscribe multiplexing, and frame routing all live in the client; this provider just
/// owns one instance for its subtree and surfaces `subscribe` + `readyState` to the monitoring
/// cards. `url` is stable for the provider's lifetime (the endpoint is edited on a different page),
/// so a live endpoint switch is handled by remounting the provider with `key={url}`.
